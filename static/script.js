...
// Consolidate your DOMContentLoaded event listeners
document.addEventListener('DOMContentLoaded', function() {
    console.log("DOM fully loaded - initializing components");

    // First, initialize lightbox functionality if gallery elements exist
    const lightbox = document.querySelector('.lightbox');
    const lightboxImage = document.querySelector('.lightbox-img');
    const lightboxCaption = document.querySelector('.lightbox-caption');
    const lightboxClose = document.querySelector('.lightbox-close');
    ...
    if (galleryItems.length > 0 && lightbox) {
        console.log("Gallery elements found - initializing lightbox");
        initLightbox(lightbox, lightboxImage, lightboxCaption, lightboxClose, lightboxPrev, lightboxNext, galleryItems);
    }

    // Then, check if weather element exists and initialize weather
    const weatherElement = document.getElementById('weather-temp');
    if (weatherElement) {
        console.log("Weather element found - fetching weather data");
        getPittsburghWeather();
    }
});

// Lightbox functionality moved to a separate function
function initLightbox(lightbox, lightboxImage, lightboxCaption, lightboxClose, lightboxPrev, lightboxNext, galleryItems) {
    let currentIndex = 0;
...
// Weather API integration - Fixed to avoid CORS issues
function getPittsburghWeather() {
    // Display loading message
    const weatherElement = document.getElementById('weather-temp');
    weatherElement.textContent = 'Loading weather data...';
    
    // Due to CORS restrictions with direct browser API calls, we'll use a fallback
    // Option 1: For demonstration purposes:
    const currentDate = new Date();
    const isWarm = currentDate.getMonth() >= 4 && currentDate.getMonth() <= 9;
    const randomTemp = isWarm ? Math.floor(Math.random() * 15) + 70 : Math.floor(Math.random() * 20) + 30;
    const conditions = isWarm ? 
        ['sunny', 'partly cloudy', 'clear skies', 'warm and breezy'] :
        ['cold', 'cloudy', 'overcast', 'chilly'];
    const randomCondition = conditions[Math.floor(Math.random() * conditions.length)];
    
    // Display weather (simulated)
    setTimeout(() => {
        weatherElement.textContent = `${randomTemp}°F with ${randomCondition}`;
    }, 500);
    
    // Comment: For production use, you would need one of these solutions:
    // 1. Set up a backend proxy that makes the API call for you
    // 2. Use a CORS-enabled public weather API
    // 3. Use the OpenWeatherMap JavaScript widget instead
}

// Keep the form validation separate
function validation() {
    var $firstName = $("#firstName");
    var $lastName = $("#lastName");
    var $email = $("#email");
